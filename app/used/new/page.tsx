...
import { createListing, uploadListingPhotos, ListingType } from "@/lib/used-listings";
...
export default function NewListingPage() {
  ...
  // Price — asking price when selling, per-day rate when renting
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (!data.session) {
        router.replace("/login?next=/used/new");
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!brand || !model || !year || !askingPrice || !location) {
      setError(
        `Fill in the required fields: brand, model, year, ${listingType === "rent" ? "rental price" : "asking price"}, and location.`
      );
      return;
    }
    ...
    if (Number(askingPrice) <= 0) {
      setError(
        listingType === "rent"
          ? "Enter a valid rental price per day in NPR."
          : "Enter a valid asking price in NPR."
      );
      return;
    }
    if (!sellerPhone.trim()) {
      setError(
        listingType === "rent"
          ? "Add a contact phone number so renters can reach you."
          : "Add a contact phone number so buyers can reach you."
      );
      return;
    }
      ...
      <main className="mx-auto w-full max-w-3xl flex-grow px-4 py-8 sm:px-6">
        ...
        <div className="mt-4">
          <span className="text-[11px] font-black uppercase tracking-wider text-blue-600">
            Sell or rent out your car
          </span>
          <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">
            List your car
          </h1>
          <p className="mt-1 text-sm font-medium text-slate-500">
            Your listing goes live immediately and {listingType === "rent" ? "renters" : "buyers"} contact you directly.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-6">
          ...
          <section className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3" role="radiogroup" aria-label="Listing type">
              {(
                [
                  { key: "sell", title: "Sell it", sub: "One-time sale at your asking price" },
                  { key: "rent", title: "Rent it out", sub: "Charge a per-day rental rate" },
                ] as const
              ).map((opt) => (
                <button
                  key={opt.key}
                  type="button"
                  role="radio"
                  aria-checked={listingType === opt.key}
                ...
                >
                  <span className={`block text-sm font-extrabold ${listingType === opt.key ? "text-blue-700" : "text-slate-900"}`}>
                    {opt.title}
                  </span>
                  <span className="mt-0.5 block text-xs font-medium text-slate-500">{opt.sub}</span>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <TextField
                id="price-input"
                label={listingType === "rent" ? "Rental price per day (NPR)" : "Asking price (NPR)"}
                required
                type="number"
                value={askingPrice}
                onChange={setAskingPrice}
                placeholder={listingType === "rent" ? "e.g. 8000" : "e.g. 3500000"}
                hint={
                  Number(askingPrice) > 0
                    ? `Shown as Rs. ${Number(askingPrice).toLocaleString("en-NP")}${listingType === "rent" ? " /day" : ""}`
                    : undefined
                }
              />
            </div>
          ...
          <section className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
              ...
              {photos.length < MAX_PHOTOS && (
                <label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-slate-300 text-slate-400 transition-colors hover:border-blue-400 hover:text-blue-500">
          ...
          <section className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5">
              ...
              <textarea
                ...
                rows={4}
                placeholder={`Service history, ownership, extras — anything a ${listingType === "rent" ? "renter" : "buyer"} should know.`}
                className={INPUT}
              />
            </div>
          </section>
